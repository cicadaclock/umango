...
// skillTableColumn represents SkillData's header name, rendered data,
// and row ordering for sorting
type skillTableColumn struct {
	header string
	value  func(td SkillTableData) []string
	less   func(td SkillTableData, i, j int) bool
}

// Source of truth for column order and content
var skillTableColumns = []skillTableColumn{
	{
		"ID",
		func(td SkillTableData) []string { return itoaSlice(td.TrainedCharaIds) },
		func(td SkillTableData, i, j int) bool {
			switch td.sortState {
			case Ascending:
	...
	{
		"Fielded",
		func(td SkillTableData) []string { return btoaSlice(td.Fielded) },
		func(td SkillTableData, i, j int) bool {
			switch td.sortState {
			case Ascending:
	...
	{
		"Distance",
		func(td SkillTableData) []string { return td.Distances },
		func(td SkillTableData, i, j int) bool {
			switch td.sortState {
			case Ascending:
	...
	{
		"Style",
		func(td SkillTableData) []string { return td.Styles },
		func(td SkillTableData, i, j int) bool {
			switch td.sortState {
			case Ascending:
	...
	{
		"# Races",
		func(td SkillTableData) []string { return itoaSlice(td.NumRaces) },
		func(td SkillTableData, i, j int) bool {
			switch td.sortState {
			case Ascending:
	...
	{
		"Max",
		func(td SkillTableData) []string { return itoaSlice(td.MaxScores) },
		func(td SkillTableData, i, j int) bool {
			switch td.sortState {
			case Ascending:
	...
	{
		"Avg",
		func(td SkillTableData) []string { return itoaSlice(td.AvgScores) },
		func(td SkillTableData, i, j int) bool {
			switch td.sortState {
			case Ascending:
...
func NewSkillData(dataStore SkillMapper, ttrs TeamTrialResultSet) SkillTableData {
...
// Filter returns a new SkillData containing only the rows at the given indices.
...
func (td SkillTableData) Less(i, j int) bool {
	if td.sortState == Unsorted {
		return td.origIndexes[i] < td.origIndexes[j]
	}
	return skillTableColumns[td.sortColumn].less(td, i, j)
}
...
// original order
func (td *SkillTableData) Sort(col int) {
...
func (td SkillTableData) Headers() []string {
	headers := make([]string, len(skillTableColumns))
	for i, col := range skillTableColumns {
...
func (td SkillTableData) Columns() [][]string {
	cols := make([][]string, len(skillTableColumns))
	for i, col := range skillTableColumns {
		cols[i] = col.value(td)
	}
	return cols
...
// including headers
func (td SkillTableData) ColumnWidths() []int {
	lengths := make([]int, len(skillTableColumns))
	for i, col := range skillTableColumns {
...
func (td SkillTableData) GetTrainedCharaId(row int) int {
